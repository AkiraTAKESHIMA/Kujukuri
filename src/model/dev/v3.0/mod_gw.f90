...
! gw exfilt
!===============================================================
subroutine gw_exfilt(hs_idx, gampt_ff_idx, hg_idx, exfilt_hs_tsas)
  use mod_globals
  implicit none
  ...
  real(8), intent(inout) :: gampt_ff_idx(slo_count)
  real(8), intent(inout) :: hg_idx(slo_count)
  real(8), intent(inout) :: exfilt_hs_tsas(slo_count)
  ...
  do k = 1, slo_count
    ...
    if( infilt_limit_idx(k) .gt. gampt_ff_idx(k) .and. &
        infilt_limit_idx(k) - gampt_ff_idx(k) .ge. exfilt .and. &
    ...
    hg_idx(k) = 0.d0
    if( ksg_idx(k) .gt. 0.d0 .and. infilt_limit_idx(k) .gt. 0.d0 )then
...
subroutine hg_init(hg_idx)
  use mod_globals
  implicit none
  real(8), intent(out) :: hg_idx(slo_count)
