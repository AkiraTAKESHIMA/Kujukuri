...
subroutine funcr( vr_idx, fr_idx, qr_idx )
  ...
  use mod_section, only: &
    ...
    vr2hr
  implicit none
  real(8), intent(inout) :: vr_idx(riv_count)
  real(8), intent(inout) :: fr_idx(riv_count)
  real(8), intent(inout) :: qr_idx(riv_count)

  real(8) :: hr_idx(riv_count)
  real(8) :: qr_sum_idx(riv_count)
  real(8) :: qr_div_idx(riv_count)
...
subroutine qr_calc(hr_idx, qr_idx)
  use mod_globals
  use mod_dam, only: damflg, dam_floodq
  implicit none
