...
!
!===============================================================
subroutine funcrs( hr, hs, qrs )
  use mod_section, only: &
    hr_update, &
    sec_h2b
  implicit none
  real(8), intent(inout) :: hr(:,:), hs(:,:)
  ...
  do i = 1, nx
    ...
    if( ( height(i,j) .eq. 0.d0 .and. hr_top .lt. 0.d0 ) .or.   &
     ...
     hrs = mu1 * hs_top * sqrt( 9.81d0 * hs_top ) * dt_model * len * 2.d0 / area ! modified 1.4.2.4 
    ...
    elseif( height(i,j) .gt. 0.d0 .and. hs_top .le. height(i,j) .and. &
            hr_top .le. height(i,j) .and. hr_top .ge. 0.d0 ) then
      qrs(i, j) = 0.d0
      continue
    ...
    elseif( hs_top .le. hr_top .and. hr_top .ge. height(i,j) ) then
     ...
     if( h2 / h1 .le. 2.d0 / 3.d0 ) then
       ...
       hrs = - mu2 * h1 * sqrt( 2.d0 * 9.81d0 * h1 ) * dt_model * len * 2.d0 / area ! modified v1.4.2.4
       else
       !hrs = - mu3 * h2 * sqrt( 2.d0 * 9.81d0 * (h1 - h2) ) * dt * len / area ! modified v1.4
       hrs = - mu3 * h2 * sqrt( 2.d0 * 9.81d0 * (h1 - h2) ) * dt_model * len * 2.d0 / area ! modified v1.4.2.4
     endif
    ...
    elseif( hs_top .ge. hr_top .and. hs_top .ge. height(i,j) ) then
      ...
      if( h2 / h1 .le. 2.d0 / 3.d0 ) then
        ...
        hrs = mu2 * h1 * sqrt( 2.d0 * 9.81d0 * h1 ) * dt_model * len * 2.d0 / area ! v1.4.2.4
      else
        !hrs = mu3 * h2 * sqrt( 2.d0 * 9.81d0 * (h1 - h2) ) * dt * len / area ! v1.4
        hrs = mu3 * h2 * sqrt( 2.d0 * 9.81d0 * (h1 - h2) ) * dt_model * len * 2.d0 / area ! v1.4.2.4
      endif
